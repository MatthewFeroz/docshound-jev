...
const MAX_SCAN_DEPTH: ScanOptions = {
  limit: 100,
...
export function HomePage() {
  ...
  useEffect(() => {
    setRunId(new URLSearchParams(location.search).get("run"));
  }, [location.search]);
  const [scan, setScan] = useState<ScanOptions>({ ...MAX_SCAN_DEPTH });
  const [liveUsage, setLiveUsage] = useState<UsageSummary | null>(null);
  const [operationEvents, setOperationEvents] = useState<RunEvent[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToRun(
      runId,
      (event) => {
        if (event.usage) setLiveUsage(event.usage);
  ...
  async function startRun(event: React.FormEvent) {
    ...
    if (
      (Object.keys(MAX_SCAN_DEPTH) as Array<keyof ScanOptions>).some(
        (key) =>
          !Number.isInteger(scan[key]) ||
          scan[key] < 1 ||
          scan[key] > MAX_SCAN_DEPTH[key],
      )
    ) {
      setOpenReadinessSection("model");
      setError(
        "Search depth values must be whole numbers within the displayed limits.",
      );
      return;
    }
    setStarting(true);
    ...
    try {
      ...
      void navigate(`/?run=${created.run_id}`, { replace: true });
      setLiveUsage(null);
      setOperationEvents([]);
      setRun(null);
      setLiveGaps([]);
        ...
        {!runId ? (
          <>
            <section className="hero-v2">
              ...
              <form id="run-form" className="hero-cta-form" onSubmit={startRun}>
                <input
                      ...
                      <fieldset className="scan-options">
                        ...
                        <div className="scan-options-grid">
                          <label className="scan-option">
                            <span className="scan-option-label">
                              <span>Issues &amp; merged PRs</span>
                              <small>each</small>
                            </span>
                            <span className="scan-option-input">
                              <input
                                form="run-form"
                                type="number"
                                min={1}
                                max={MAX_SCAN_DEPTH.limit}
                                required
                                value={scan.limit}
                                onChange={(event) =>
                                  setScan({
                          ...
                          <label className="scan-option">
                            ...
                            <span className="scan-option-input">
                              <input
                                ...
                                type="number"
                                min={1}
                                max={MAX_SCAN_DEPTH.repo_docs_max_files}
                                required
                                value={scan.repo_docs_max_files}
                                onChange={(event) =>
                                  setScan({
                                    ...scan,
                              ...
                              />
                              <span aria-hidden="true">
                                / {MAX_SCAN_DEPTH.repo_docs_max_files}
                              </span>
                            </span>
                          </label>
                          <label className="scan-option">
                            <span className="scan-option-label">
                              <span>Semantic passages</span>
                            ...
                            <span className="scan-option-input">
                              <input
                                ...
                                type="number"
                                min={1}
                                max={MAX_SCAN_DEPTH.nvidia_embed_max_passages}
                                required
                                value={scan.nvidia_embed_max_passages}
                                onChange={(event) =>
                                  setScan({
                                    ...scan,
                              ...
                              <span aria-hidden="true">
                                /{" "}
                                {MAX_SCAN_DEPTH.nvidia_embed_max_passages.toLocaleString()}
                              </span>
                            </span>
                          </label>
                        </div>
