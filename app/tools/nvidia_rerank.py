...
async def rerank_gap(
    ...
    def result(chunks: list[RetrievedChunk], status: str, reason: str) -> RerankResult:
        return RerankResult(chunks, status, reason, settings.nvidia_rerank_model, len(candidates))
...
def _validated_ranking(payload: object, candidates: list[RetrievedChunk]) -> list[RetrievedChunk]:
