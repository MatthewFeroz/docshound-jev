...
async def observe_operation(
    ...
    *args: Any,
    ...
    output_details: (Mapping[str, Any] | Callable[[Any], Mapping[str, Any]] | None) = None,
    ...
    **kwargs: Any,
    ...
    async with langsmith_trace(
        ...
        try:
            ...
            traced_outputs = (
                trace_outputs(result) if trace_outputs else {"result": summary_out, **details_out}
            )
...
def _default_input_summary(
    ...
    if name == "cluster_issues":
        return (
            f"{details.get('issues', 0)} issues · {details.get('pull_requests', 0)} pull requests"
        )
    if name == "draft_findings":
